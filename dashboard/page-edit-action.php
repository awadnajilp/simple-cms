...
<?php if (isset($_POST['submit_form'])) {
	...
	$sql = "UPDATE `pages` set `title`=$title, `content`=$content, `date`='$date',`featured_img`='$ft_fileName' where id=$page_id";
	mysql_query($sql);
	    ...
	    if(isset($_REQUEST['AddFiles'])){
			...
			if ($upload_flag == 1) {
						header("location: page-edit.php?id=$page_id&succ");
			}
}else{
	header("location: page-edit.php?error=1");
}?>