...
function docReady(){
	...
	//rich text editor
	$('.cleditor').cleditor({width : '100%' , height : '300'});
	...
	$('.thumbnails').on('click','.gallery-delete',function(e){
		...
		if(confirm('Are you sure?')){
			e.preventDefault();
		var id = $(this).parents('.thumbnail').attr('id');
		/*$.ajax({type:'POST',url:"../delete-upload-image.php",data:{id:id},success:function(data){
			alert(data);
		}});*/
		$.post( "delete-upload-image.php",{id:id}, function( data ) {
		  if(data){
		  	$('#'+id).fadeOut();
		  }else{
		  	alert("Error Ocuured!");
